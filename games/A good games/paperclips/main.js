...
function exportGame() {
    ...
    var bundle = {
        ...
        savePrestige: JSON.parse(localStorage.getItem("savePrestige"))
    };
    
    var code = btoa(unescape(encodeURIComponent(JSON.stringify(bundle))));
    document.getElementById("saveCodeDiv").style.display = "block";
...
function importGame() {
    ...
    if (code) {
        try {
            var bundle = JSON.parse(decodeURIComponent(escape(atob(code))));
            if (bundle.saveGame) localStorage.setItem("saveGame", JSON.stringify(bundle.saveGame));
            if (bundle.saveProjectsUses) localStorage.setItem("saveProjectsUses", JSON.stringify(bundle.saveProjectsUses));
            if (bundle.saveProjectsFlags) localStorage.setItem("saveProjectsFlags", JSON.stringify(bundle.saveProjectsFlags));
            if (bundle.saveProjectsActive) localStorage.setItem("saveProjectsActive", JSON.stringify(bundle.saveProjectsActive));
            if (bundle.saveStratsActive) localStorage.setItem("saveStratsActive", JSON.stringify(bundle.saveStratsActive));
            if (bundle.savePrestige) localStorage.setItem("savePrestige", JSON.stringify(bundle.savePrestige));
...
function copySaveCode() {
    var copyText = document.getElementById("saveCodeArea");
    copyText.select();
